                ...
                <tr class="bg-white hover:bg-gray-50">
                    ...
                    <td class="py-2 border border-gray-200 text-center p-4 w-32">
                        <div class="flex flex-col gap-1">
                            <a href="{{route('surat-keluar.admin.show', $surat->id)}}" class="block text-center px-2 py-1 bg-green-500 border border-gray-300 rounded-md text-gray-100 text-xs md:text-sm hover:bg-green-400">
                                Detail
                            </a>
                            <a href="{{route('surat-keluar.admin.edit',$surat->id)}}" class="block text-center px-2 py-1 bg-gray-500 text-white rounded-md text-xs md:text-sm hover:bg-gray-700">
                                Edit
                            </a>
                            <form action="{{route('surat-keluar.admin.destroy', $surat->id)}}" method="POST">
                                @csrf
                                @method('DELETE')
